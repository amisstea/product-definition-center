...
class ReleasedFilesRESTTestCase(TestCaseWithChangeSetMixin, APITestCase):
    ...
    def setUp(self):
        self.data = {"file_primary_key": 3, "repo": 1, "release_date": "2017-11-13"}
        self.existing = {
            "file_primary_key": 2,
            "repo": 3,
    ...
    def test_get(self):
        ...
        response = self.client.get(reverse('releasedfiles-list'), {'file': "bash-1.2.3-4.b1.x86_64.rpm"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["count"], 1)
    ...
    def test_create(self):
        response = self.client.post(reverse('releasedfiles-list'), self.existing)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
